...
import { CreateProducerService, DeleteProducerService, DetailProducerService, ListAllProducersService, UpdateProducerService } from '../src/modules/producers/application/services';
...
describe('ProducersController (e2e)', () => {
  ...
  beforeEach(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: ListAllProducersService, useValue: ListAllProducersServiceMock }, 
        { provide: DetailProducerService, useValue: DetailProducerServiceMock },
        { provide: CreateProducerService, useValue: CreateProducerServiceMock },
        { provide: UpdateProducerService, useValue: UpdateProducerServiceMock },
        { provide: DeleteProducerService, useValue: DeleteProducerServiceMock },
      ]
    }).compile();
  ...
  it('/:id (DELETE)', async () => {
    const { body: responseProducers } = await apiClient().get('/producers')
    const { body: responseDelete } = await apiClient().delete(`/producers/${responseProducers.producers[0].id}`).expect(200)
  ...
  it('/:id (UPDATE)', async () => {
    ...
    const { body: response } = await apiClient().patch(`/producers/${responseProducers.producers[0].id}`).send({ name: 'Alberto Senna' }).expect(200)
